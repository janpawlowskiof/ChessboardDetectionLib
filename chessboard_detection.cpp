...
cv::Mat perspective_transform(cv::Mat &image, cv::Vec2f top_l, cv::Vec2f top_r, cv::Vec2f bottom_l, cv::Vec2f bottom_r, int output_size)
{
    ...
    outputQuad[1] = cv::Point2f(output_size,0);
    outputQuad[2] = cv::Point2f(output_size,output_size);
    outputQuad[3] = cv::Point2f(0,output_size);

    auto matrix = cv::getPerspectiveTransform(inputQuad, outputQuad);

    cv::Mat output;
    cv::warpPerspective(image, output, matrix, cv::Size(output_size, output_size));
    return output;
}
...
bool split_image_into_folder(cv::Mat &image, IntersectionsVec &intersections, std::string path)
{
    ...
    for(int i = 0; i < 8; i++)
    {
        for(int j = 0; j < 8; j++)
        {
            auto cell = perspective_transform(image, intersections[i+1][j], intersections[i+1][j+1],
                                              intersections[i][j], intersections[i][j+1], 32);
